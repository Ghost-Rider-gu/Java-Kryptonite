...
public class LaserBeamsBank {
    ...
    public static int numberOfBeams(String[] bank) {
        int result = 0;
        List<Integer> stringWithBeams = new ArrayList<>();

        for (String string : bank) {
            if (withBeams(string)) {
                stringWithBeams.add(beamCount(string));
            }
        }

        if (stringWithBeams.size() <= 1) {
            return 0;
        }

        for (int i = 0; i < stringWithBeams.size() - 1; i++) {
            result += stringWithBeams.get(i) * stringWithBeams.get(i + 1);
        }

        return result;
    }

    private static boolean withBeams(String stringWithBeams) {
        return stringWithBeams.contains("1");
    }

    private static int beamCount(String stringWithBeams) {
        return stringWithBeams.replaceAll("0", "").length();
    }
}
